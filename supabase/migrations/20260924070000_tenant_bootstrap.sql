...
declare
  ...
  safe_name text := trim(coalesce(hotel_name, ''));
...
begin
  ...
  if safe_currency !~ '^[A-Z]{3}(hashtextextended(user_uuid::text || ':' || safe_key, 0));

  select r.request_fingerprint into existing_fingerprint
  ...
  if existing_fingerprint is not null then
    ...
    return jsonb_build_object(
      'created', false,
      'hotel', jsonb_build_object('id',existing_hotel.id,'slug',existing_hotel.slug,'name',existing_hotel.name,'timezone',existing_hotel.timezone,'currency',existing_hotel.currency,'automation_paused',existing_hotel.automation_paused),
      'role', 'owner'
    );
  end if;
  ...
  if existing_hotel.id is not null then
    if exists (
      ...
      return jsonb_build_object(
        'created', false,
        'hotel', jsonb_build_object('id',existing_hotel.id,'slug',existing_hotel.slug,'name',existing_hotel.name,'timezone',existing_hotel.timezone,'currency',existing_hotel.currency),
        'role', 'owner'
      );
    end if;
  ...
  values(safe_slug,safe_name,safe_timezone,safe_currency,true,jsonb_build_object('onboarding','server-bootstrap-v1','server_authority','disabled'))
  ...
  values(new_hotel.id,user_uuid,'user','Governance','Hotel bootstrapped','Initial Owner tenant bootstrap completed',jsonb_build_object('slug',safe_slug,'automation_rules',12));
  ...
  return jsonb_build_object(
    'created', true,
    'hotel', jsonb_build_object('id',new_hotel.id,'slug',new_hotel.slug,'name',new_hotel.name,'timezone',new_hotel.timezone,'currency',new_hotel.currency,'automation_paused',new_hotel.automation_paused),
    'role', 'owner',
...
comment on function public.bootstrap_hotel_owner(uuid,text,text,text,text,text)
  is 'Service-role only. Transactionally creates a hotel, first Owner membership, default automation rules and audit record. Idempotent per authenticated user + key.';
),
...
begin
  ...
  if existing_hotel.id is not null then
    if exists (
      ...
      insert into private.hotel_bootstrap_requests(user_id,idempotency_key,hotel_id)
      values(user_uuid,safe_key,existing_hotel.id)
      on conflict do nothing;
      return jsonb_build_object(
        'created', false,
        'hotel', jsonb_build_object('id',existing_hotel.id,'slug',existing_hotel.slug,'name',existing_hotel.name,'timezone',existing_hotel.timezone,'currency',existing_hotel.currency),
  ...
  insert into public.hotels(slug,name,timezone,currency,settings)
  values(safe_slug,safe_name,safe_timezone,safe_currency,jsonb_build_object('onboarding','server-bootstrap-v1'))
  returning * into new_hotel;
  ...
  insert into public.automation_rules(hotel_id,rule_key,name,scope,event_type,status,autonomy,config)
  values
    (new_hotel.id,'reservation-intake','Reservation intake','Operations','reservation.created','Active','Auto','{"trigger":"Reservation received","action":"Hold inventory → reconcile channels → confirmation"}'),
    (new_hotel.id,'checkout-turnover','Checkout turnover','Operations','guest.checked_out','Active','Auto','{"trigger":"Guest checked out","action":"Room → Dirty → housekeeping task → sellability update"}'),
    (new_hotel.id,'pre-arrival-message','Pre-arrival message','Operations','prearrival.due','Active','Auto','{"trigger":"24h before arrival","action":"Send arrival instructions → track delivery"}'),
    (new_hotel.id,'occupancy-rate-guard','Occupancy rate guard','Revenue','occupancy.threshold','Active','Policy','{"trigger":"Occupancy > 80%","action":"BAR change → policy check → apply or approve"}'),
    (new_hotel.id,'failed-payment-recovery','Failed payment recovery','Finance','payment.failed','Paused','Auto','{"trigger":"Payment authorization fails","action":"Retry → flag folio → create exception"}'),
    (new_hotel.id,'low-stock-replenishment','Low-stock replenishment','Operations','inventory.low_stock','Active','Policy','{"trigger":"Item falls below par","action":"Calculate reorder → policy check → PO / approval"}'),
    (new_hotel.id,'cancellation-recovery','Cancellation recovery','Operations','reservation.cancelled','Active','Auto','{"trigger":"Reservation cancelled","action":"Release inventory → reconcile channels → resale"}'),
    (new_hotel.id,'room-ready-release','Room-ready release','Operations','housekeeping.completed','Active','Auto','{"trigger":"Housekeeping marks room ready","action":"Set Clean → recalculate sellability → channel release"}'),
    (new_hotel.id,'room-conflict-guard','Room conflict guard','Operations','room.maintenance_blocked','Active','Approval','{"trigger":"Assigned room goes out of order","action":"Find affected stay → alternatives → exception"}'),
    (new_hotel.id,'guest-request-router','Guest request router','Operations','guest.request_received','Active','Auto','{"trigger":"Guest service request received","action":"Classify → create task → route team"}'),
    (new_hotel.id,'approval-executor','Approval executor','Governance','approval.approved','Active','Auto','{"trigger":"Owner approves an action","action":"Execute approved action → close handoff → audit"}'),
    (new_hotel.id,'review-recovery','Review recovery','Guest experience','review.negative','Active','Approval','{"trigger":"Low guest feedback detected","action":"Create recovery workflow → notify manager"}');
  ...
  insert into private.hotel_bootstrap_requests(user_id,idempotency_key,hotel_id)
  values(user_uuid,safe_key,new_hotel.id);

  return jsonb_build_object(
    'created', true,
    'hotel', jsonb_build_object('id',new_hotel.id,'slug',new_hotel.slug,'name',new_hotel.name,'timezone',new_hotel.timezone,'currency',new_hotel.currency),
    'role', 'owner',
    'automation_rules_seeded', 12
  );
end;
