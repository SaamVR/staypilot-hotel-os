...
create or replace function private.ezstay_start_demo_command(
  target_user_id uuid,
...
declare
  ...
  request_hash text := encode(digest('ezstay:demo.start:v1', 'sha256'), 'hex');
...
begin
  ...
  insert into platform.demo_command_idempotency (
    ...
    ezstay_app_id, target_user_id, target_idempotency_key, 'demo.start', request_hash
  )
  ...
  if command_row.command_type <> 'demo.start'
  ...
  if command_row.response ? 'session_id' then
    ...
    if result_session.id is not null then
      return result_session;
    end if;
  end if;

  result_session := private.create_ezstay_demo_session(target_user_id);
...
revoke execute on function private.ezstay_start_demo_command(uuid, text) from public, anon, authenticated;
